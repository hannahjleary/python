...
datadir = '../../../../../ix/eschneider/hjl28/data/tests/cloud_tracking/hdf5_large_ct/'
csvdir = '../../../../../ix/eschneider/hjl28/data/tests/cloud_tracking/hdf5_large_ct/'

sims = ['4/'] #, '4/', '8/', '16/', '32/', '48/'
cat = [False, False, True, True, True] # False, False, True, True, True
...
if MASS:
     filename = "dm_" + str(cloud_thresh) + ".csv"
if VELOCITY:
     filename = "dv_" + str(cloud_thresh) + ".csv"
...
for i in range(len(sims)):
    for j in range(ns, ne, nstep):
        ...
        if cat[i]:
            ...
            f = h5py.File(datadir + 'raw/' + str(j) + '/' + str(j) + '.h5.0', 'r') 
        head = f.attrs
        t = head['t'][0]
