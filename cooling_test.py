...
for i in range(istart, iend):
    ...
    subplots = [logT.T, logn.T, Z_sol.T] #subplots = [logT.T, P.T, Vx.T]
    mins = [Tmin, nmin, Zmin]
    maxs = [Tmax, nmax, Zmax]
    cmaps = ['plasma', 'viridis', 'magma_r']
    labels = ['$log_{10}(K)$', '$log_{10}(N_{H})$ [$cm^{-2}$]', '$Z [Z_{\odot}]$']
    ...
    for j in range(len(subplots)):
        ...
        axs[j].set_xticks(np.linspace(0,nx,5))
        ...
        if j == (len(subplots)-1):
            ...
            axs[j].set_xticklabels(np.round(np.linspace(0,nx*dx*1000+.01, 5),1))
            # print(nx*dx)
            [l.set_visible(False) for (i,l) in enumerate(axs[j].xaxis.get_ticklabels()) if i % 2 != 0]
            axs[j].set_xlabel('$pc$', size=8, color=fig_color)
        else:
            axs[j].tick_params(axis='both', which='both', direction='in', color=fig_color, bottom=1, left=1, top=1, right=1, 
